...
def draw_flow_mask(frame, flow, mask, debug=False):
    ...
    dilated_mask = cv2.dilate(mask, np.ones((11, 11), np.uint8), iterations=2)
    ...
    for i in range(len(Ox)):
        for j in range(len(Ox[i])):
            ...
            cy = int(Cy[i][j])
            #
            # # Ensure cx and cy indices are within the bounds of the dilated_mask dimensions
            if cx >= 0 and cy >= 0 and cy < dilated_mask.shape[0] and cx < dilated_mask.shape[1]:
                if dilated_mask[int(Cy[i][j]), int(Cx[i][j])] == 255:
                    dx = cx - int(Ox[i][j])
                    dy = cy - int(Oy[i][j])
                    pt1 = (cx, cy)
    ...
    if debug:
        cv2.imshow("Flow Hull", drawn_frame_and)
